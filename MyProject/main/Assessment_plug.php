...
if ($action == "search_emp") { // table of employee after searching // done -2
    $sql = "select * from t_memc_kpcc_employee_detail,t_memc_department where EmpDetail_Status = 1 and Emp_D_ID = dpt_id";
    if ($_POST['EID'] != null) {
            ...
            <tr>
                ...
                <td scope="col">
                    ...
                    $group_talent = $conn->query("Select * from t_memc_kpcc_employee_category,t_memc_kpcc_category 
                        where ec_employee_id = '" . $row['Emp_ID'] . "' and ec_category_id = c_id");
...
if ($next == "66%" || $previous == "66%") {
    ...
    $_SESSION['Action_Plan_ID'][$_SESSION['start'] - 1] = $_SESSION['start'];
...
?>
    ...
    <div class="col-12">
            ...
            <tr>
                ...
                <td style="width: 40%;">
                    <select class="form-control" id="edit_year" name="edit_year">
                        <?php
                        $a = "select DISTINCT Q_Year from t_memc_kpcc_quarter order by Q_Year desc";
                        $year_data = $conn->query($a);
                        $newest_year = 0;
                        while ($temp = $year_data->fetch_assoc()) {
                            if ($newest_year == 0) {
                                $newest_year = $temp['Q_Year'];
                            }
                        ?>
                ...
                <td style="width: 40%;">
                    <select class="form-control" name="edit_quarter" id="edit_quarter">
                        <option value="all">All</option>
                        <?php
                        $quar = $conn->query("select * from t_memc_kpcc_quarter where Q_Year = '" . $newest_year . "' order by Q_Name Desc");
                        $newest_quarter = 0;
                        while ($temp = $quar->fetch_assoc()) {

                        ?>
                            <option <?php if ($newest_quarter == 0) {
                                        echo "selected";
                                    } ?> value="<?php echo $temp['Q_Name']; ?>"><?php echo $temp['Q_Name']; ?></option>
                        <?php
                            if ($newest_quarter == 0) {
                                $newest_quarter = $temp['Q_Name'];
                            }
                        }
                        ?>
                    </select>
                </td>
                <td style="text-align: center;"><button type="button" class="btn btn-primary" style="width: 80%;" onclick="SearchPassingData()">Search</button></td>
            </tr>
        </table>
        <table class="table table-bordered mt-3">
            ...
            <tbody id="Quarter_Core">
                <?php
                InEditSearchEmployee(date('Y'), $newest_quarter, $conn, $_POST['empid']);
                ?>
            </tbody>
        </table>
    </div>
...
if ($action == "InEdit_Search_Emp") {
    InEditSearchEmployee($_POST['selected_year'], $_POST['selected_quarter'], $conn, $_POST['empid']);
}
...
if ($action == "removeCardAP") {
    unset($_SESSION['Action_Plan'][$_POST['n_start'] - 1][$_POST['Num']]);
}

function AddCardActionPlan_func($conn, $num, $type, $data)
{
    $_SESSION['Action_Plan'][$num - 1][$_SESSION['total_AP']] = $type;
    ?>
    <div class="card mt-2" id="cardAP_IN<?php echo $_SESSION['total_AP']; ?>">
        <div class="card-header" style="text-align: right;">
            <span style="width:90%"><?php if ($type == "insert") {
                                        echo "New";
                                    } else {
                                        echo "Edit";
                                    } ?> Action Plan</span>
            <span class="oi oi-x" id="remove_AP<?php echo $_SESSION['total_AP']; ?>" onclick="removeCardAP('<?php echo $_SESSION['total_AP']; ?>','<?php echo $_SESSION['start']; ?>')" style="width:90%;cursor:pointer"></span>
        </div>
        <div class="card-body" id="card_bodyAP_<?php echo $num; ?>">
            <div class="row">
                <div class="col-9">
                    <textarea name="AP_Desc<?php echo $num; ?>[]" class="form-control" style="width: 100%;height:130px;overflow-y:scroll" rows="30"><?php if ($type != "insert") {
                                                                                                                                                        echo $data['AP_Description'];
                                                                                                                                                    } ?></textarea>
                </div>
                <div class="col-3">
                    <div class="row">
                        <div class="col-3"><label class="col-form-label">Date</label></div>
                        <div class="col-9"><input type="date" name="AP_Date<?php echo $num; ?>[]" style="max-width: 100%;" class="form-control" value="<?php if ($data['AP_Date'] != null) {
                                                                                                                                                            echo $data['AP_Date'];
                                                                                                                                                        } ?>"></div>
                        ...
                        <div class="col-9">
                            <select name="AP_Project<?php echo $num; ?>[]" style="width: 100%;" class="form-control mt-2">
                                ...
                                ?>
                                    <option value="<?php echo $row['Pt_ID']; ?>" <?php if ($data['AP_Pt_ID'] == $row['Pt_ID']) {
                                                                                        echo "selected";
                        ...
                        <div class="col-9">
                            <select name="AP_Status<?php echo $num; ?>[]" style="width: 100%;" class="form-control mt-2">
                                <option value="1" <?php if ($data['AP_Status'] == "1") {
                                                        echo "selected";
                                                    } ?>>Open</option>
                                <option value="0" <?php if ($data['AP_Status'] == "0") {
                                                        echo "selected";
                                                    } ?>>Close</option>
                                <option value="2" <?php if ($data['AP_Status'] == "2") {
                                                        echo "selected";
                                                    } ?>>Cancel</option>
                            </select>
                        </div>
...
function InEditSearchEmployee($year, $newest_quarter, $conn, $empid)
{
    ...
    if ($newest_quarter != "all") {
        $sql .= " and Q_Name = '" . $newest_quarter . "'";
    }
    $sql .= " order by Q_Name DESC,Q_Year DESC, Im_Name ASC";
    $result = $conn->query($sql);
    $row = mysqli_num_rows($result);
    if ($row == 0) {
    ?>
