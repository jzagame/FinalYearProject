...
if ( $_POST[ 'action' ] == "creatept" ) {
  ...
  if ( mysqli_num_rows( $result2 ) > 0 ) {
    for ( $i = 0; $i < mysqli_num_rows( $result2 ); ++$i ) {
      $row2 = mysqli_fetch_array( $result2 );
      if ( $formdata[ 0 ][ 'value' ] == $row2[ 'Pt_Name' ] ) {
        $xx = 1;
      }
    }
  }
...
if ( $_POST[ 'action' ] == "editpt" ) {
  ...
  if ( mysqli_num_rows( $result2 ) > 0 ) {
    for ( $i = 0; $i < mysqli_num_rows( $result2 ); ++$i ) {
      $row2 = mysqli_fetch_array( $result2 );
      if ( $formdata[ 0 ][ 'value' ] == $row2[ 'Pt_Name' ] && $formdata[ 1 ][ 'value' ] == $row2[ 'Pt_Status' ] ) {
        $xx = 1;
      }
    }
  }
...
if ( $_POST[ 'action' ] == "searchpt" ) {
  ...
  if ( trim( $formdata[ 1 ][ 'value' ] ) != "" ) {
    $sql .= "Pt_Status = '" . trim( $formdata[ 1 ][ 'value' ] ) . "' AND ";
  }
  $sql .= "ORDER BY Pt_ID";
  $sql = str_replace( "AND ORDER", "ORDER", $sql );
  $view = mysqli_query( $conn, $sql );
